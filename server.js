...
// ===== Database Connection Helpers (safe for demo without PostgreSQL) =====
...
// ===== Simple In-Memory Rate Limiter (for POST /api/orders) =====
...
function simpleRateLimit(req, res, next) {
  ...
  if (entry.count > RATE_LIMIT_MAX) {
    return res.status(429).json({
      success: false,
      message: 'طلبات كثيرة جداً. الرجاء الانتظار قليلاً قبل إرسال طلب جديد.'
    });
  }
  next();
}
...
// Shared PDF footer
function pdfFooter() {
  return `<div class="footer"><div class="footer-text">شكراً لتعاملكم معنا</div><div class="footer-sub">FoodExpress</div></div>`;
}
...
/* =========================
   API: TABLES
========================= */
const tablesFile = 'tables.json';

function readTables() {
  return jsonStore.readJsonFile(tablesFile);
}

function writeTables(data) {
  return jsonStore.writeJsonFile(tablesFile, data);
}

app.get('/api/tables', requirePerm('view_orders'), (req, res) => {
  res.json(readTables());
});

app.post('/api/tables', requirePerm('manage_categories'), (req, res) => {
  const { code, name, capacity } = req.body;
  if (!code || !String(code).trim()) {
    return res.status(400).json({ success: false, message: 'رمز الطاولة مطلوب' });
  }
  const parsedCap = Number(capacity);
  if (capacity !== undefined && (!Number.isInteger(parsedCap) || parsedCap < 1)) {
    return res.status(400).json({ success: false, message: 'السعة يجب أن تكون رقماً صحيحاً موجباً' });
  }
  const tables = readTables();
  const exists = tables.find(t => String(t.code).toLowerCase() === String(code).trim().toLowerCase());
  if (exists) {
    return res.status(400).json({ success: false, message: 'رمز الطاولة موجود مسبقاً' });
  }
  const newTable = {
    id: String(code).trim().toUpperCase(),
    code: String(code).trim().toUpperCase(),
    name: name || 'طاولة ' + code,
    capacity: parsedCap || 4,
    active: true,
  ...
  writeTables(tables);
  res.json({ success: true, data: newTable, message: 'تمت إضافة الطاولة' });
});

app.put('/api/tables/:id', requirePerm('manage_categories'), (req, res) => {
  const tables = readTables();
  const idx = tables.findIndex(t => String(t.id) === String(req.params.id));
  if (idx === -1) {
    return res.status(404).json({ success: false, message: 'الطاولة غير موجودة' });
  }
  const { code, name, capacity, active } = req.body;
  if (code !== undefined) {
    const codeStr = String(code).trim().toUpperCase();
    if (!codeStr) {
      return res.status(400).json({ success: false, message: 'رمز الطاولة لا يمكن أن يكون فارغاً' });
    }
    const dup = tables.find(t => String(t.code).toLowerCase() === codeStr.toLowerCase() && String(t.id) !== String(req.params.id));
    if (dup) {
      return res.status(400).json({ success: false, message: 'رمز الطاولة موجود مسبقاً' });
    }
    tables[idx].code = codeStr;
  }
  ...
  if (capacity !== undefined) {
    ...
    tables[idx].capacity = parsedCap;
  }
  if (active !== undefined) tables[idx].active = Boolean(active);
  tables[idx].updatedAt = new Date().toISOString();
  writeTables(tables);
  res.json({ success: true, data: tables[idx], message: 'تم تحديث الطاولة' });
});

app.delete('/api/tables/:id', requirePerm('manage_categories'), (req, res) => {
  const tables = readTables();
  const idx = tables.findIndex(t => String(t.id) === String(req.params.id));
  if (idx === -1) {
    return res.status(404).json({ success: false, message: 'الطاولة غير موجودة' });
  ...
  const hasActiveOrders = allOrders.some(function(o) {
    return String(o.tableNumber) === String(table.code) && ['pending', 'confirmed', 'processing', 'shipped'].includes(o.status);
  });
  if (hasActiveOrders) {
    return res.status(400).json({ success: false, message: 'لا يمكن حذف طاولة عليها طلبات نشطة. قم بإنهاء الطلبات أولاً.' });
  }
  const removed = tables.splice(idx, 1)[0];
  writeTables(tables);
  res.json({ success: true, data: removed, message: 'تم حذف الطاولة' });
});
...
<body>
  ...
  <div class="footer"><div class="footer-text">شكراً لتعاملكم معنا</div><div class="footer-sub">FoodExpress</div></div>
</body></html>`;
...
/* =========================
   API: ORDERS
========================= */
app.get('/api/orders', requirePerm('view_orders'), (req, res) => {
...
app.post('/api/orders', simpleRateLimit, (req, res) => {
  try {
...
app.get('/api/direct-sales', requirePerm('view_orders'), (req, res) => {
  try {
    const directSales = readJSON('direct-sales.json');
...
app.put('/api/direct-sales/:id', requirePerm('update_orders'), (req, res) => {
  try {
...
app.put('/api/direct-sales/:id/cancel', requirePerm('update_orders'), (req, res) => {
  try {
    const saleId = Number(req.params.id);
...
========================= */
app.post('/api/stock-receipts', requirePerm('update_orders'), (req, res) => {
...
app.get('/api/stock-receipts', requirePerm('view_orders'), (req, res) => {
  try {
    const stockReceipts = readJSON('stock-receipts.json');
...
app.post('/api/login', async (req, res) => {
  ...
  if (hasDatabaseUrl() && prisma) {
    try {
      const company = await withTimeout(prisma.company.findFirst(), 2000);
      companyId = company ? company.id : null;
    } catch (err) {
      console.warn('Prisma query skipped or failed (continuing without companyId):', err.message);
    }
  }
  const payload = {
    userId: user.id,
...
/* =========================
   API: IMAGE UPLOAD
========================= */
app.post('/api/upload', requirePerm('edit_products'), (req, res) => {
...
========================= */
app.get('/api/accounting/customers', requirePerm('view_dashboard'), async (req, res) => {
...
app.post('/api/accounting/customers', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
      return res.status(404).json({ success: false, message: 'No company found in the database.' });
...
app.put('/api/accounting/customers/:id', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
      return res.status(404).json({ success: false, message: 'No company found in the database.' });
...
========================= */
app.get('/api/accounting/suppliers', requirePerm('view_dashboard'), async (req, res) => {
...
app.post('/api/accounting/suppliers', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
      return res.status(404).json({ success: false, message: 'No company found in the database.' });
...
app.put('/api/accounting/suppliers/:id', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
      return res.status(404).json({ success: false, message: 'No company found in the database.' });
...
========================= */
app.get('/api/accounting/accounts', requirePerm('view_dashboard'), async (req, res) => {
...
app.post('/api/accounting/accounts', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
      return res.status(404).json({ success: false, message: 'No company found in the database.' });
...
app.put('/api/accounting/accounts/:id', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
      return res.status(404).json({ success: false, message: 'No company found in the database.' });
...
// =========================
app.get('/api/accounting/settings', requirePerm('view_dashboard'), async (req, res) => {
  try {
...
app.patch('/api/accounting/settings', requirePerm('view_settings'), async (req, res) => {
  try {
    // Determine companyId from JWT or fallback
    let companyId = req.user?.companyId;
    if (!companyId) {
...
// =========================
app.post('/api/accounting/bank-deposit', requirePerm('view_settings'), async (req, res) => {
  try {
    const company = await prisma.company.findFirst();
    if (!company) {
